...
if __name__ == '__main__':
    ...
    ROAD_MESH_PATH = os.path.join(args.root_path, 'tmp_road_mesh')
    ...
    VISUALIZATION_PATH = os.path.join(args.root_path, "visualizations", "tmp_road_mesh")
    if not os.path.isdir(VISUALIZATION_PATH):
        os.mkdir(VISUALIZATION_PATH)

    counter = 0
    frame_names = [os.path.splitext(filename)[0] for filename in os.listdir(ROAD_MESH_PATH)]
    ...
    for frame_name in sorted(frame_names):
        ...
        road.paint_uniform_color([0,0,0.5])
        visuals = [pcd, road]
        road_mesh = o3d.io.read_triangle_mesh(os.path.join(ROAD_MESH_PATH, "{}.ply".format(frame_name)))
